...
async function carregaListaAutomatica() {
    try {
        const response = await fetch('js/listaDePalavras.json');
        if (!response.ok) {
            throw new Error('Falha ao carregar o arquivo JSON');
        }
        const listaDePalavras = await response.json();
        palavras = listaDePalavras; // Atribui a lista de palavras à variável global
    } catch (error) {
        console.error(error);
    }
}
...
function comparalistas(letra){
    ...
    if(vitoria == true)
    {
        pontos+=100;
        atualizarPlacar();
        abreModal("PARABÉNS!", `Você tem ${pontos} pontos...`);
        tentativas = 0;
        piscarBotaoJogarNovamente(true);
    }
}
